...
class ElevationMapper:
    ...
    def __init__(self, street_network:StreetNetwork, threads_available=mp.cpu_count()):
    ...
    def generic_get_elevation_function(self, total_nodes, idx, x_y:tuple):
        ...
        try:
            ...
            ### NEED TO FIX NODE COUNTER: need to make so counts total rather than for each thread (I think?)
            self.node_counter += 1 # adds one to the count of how many nodes have been processed
            print(f"\rElevation added for {self.node_counter}/{total_nodes} nodes", end='', flush=True)
    ...
    def add_elevation_data_to_nodes(self):
        ...
        if not self.elevation_cache.check_if_cache_already_exists():
            start_time = time.perf_counter()
            ...
            with mp.Pool(processes=self.threads_available) as pooled_threads:
                z_values = pooled_threads.starmap(self.generic_get_elevation_function,
                    zip(repeat(number_of_nodes_total), idx_args_for_elevation, x_y_args_for_elevation)
                )
...
if __name__ == "__main__":
    pinole_street_network = StreetNetwork("Pinole, California, USA")
    pinole_street_network_with_elevation = ElevationMapper(pinole_street_network)
    print(pinole_street_network_with_elevation.add_elevation_data_to_nodes())
